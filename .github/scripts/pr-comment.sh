...
#
# The per-arch build/upload matrix finishes legs independently, so the `report`
# job collects which arches actually reached the channel and renders the comment
# here (instead of inline in a single job).
#
# Usage:  pr-comment.sh <channel> <wanted-csv> <published-csv>
#   <wanted-csv>    archs the build targeted, e.g. "amd64,arm64"
#   <published-csv> archs that reached the channel (may be empty)
# Env:    SNAP_NAME (default home-assistant)
set -euo pipefail

chan="${1:?usage: pr-comment.sh <channel> <wanted-csv> <published-csv>}"
wanted="${2:-}"
published="${3:-}"
snap="${SNAP_NAME:-home-assistant}"
