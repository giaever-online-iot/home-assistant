...
#   latest-targets <new> <cand> <edge> echo latest/candidate &/or latest/edge, each gated by new>=that channel
set -euo pipefail
...
# true (exit 0) iff $1 >= $2; empty floor ("") => true. Leading 'v' optional (via _vercmp).
_at_least() {
  [ -z "${2:-}" ] && return 0
  [ "$(_vercmp "${1:-}" "$2")" -ge 0 ]
}
...
case "$cmd" in
  ...
  is-at-least)
    ...
    ;;
  latest-targets)
    # args: <new-ver> <latest/candidate ver> <latest/edge ver>. Echo the latest/* channels
    # <new-ver> may land on — each gated independently by <new-ver> >= that channel's OWN
    # current version, so a backport can never downgrade a shared channel already ahead.
    # Empty floor (channel currently empty) => qualifies.
    v="${1:-}"; out=""
