...
func TestCreatesWhenMissing(t *testing.T) {
	...
	if act, _ := Reconcile(f, mustCfg(), false); act != ActionCreated || !f.ran || !f.pulled {
...
func TestRecreatesOnHashMismatch(t *testing.T) {
	...
	if act, _ := Reconcile(f, mustCfg(), false); act != ActionRecreated || !f.removed || !f.ran || !f.pulled {
...
func TestNoOpWhenMatchingAndRunning(t *testing.T) {
	...
	if act, _ := Reconcile(f, c, false); act != ActionNone || f.pulled {
...
func TestStartsWhenStopped(t *testing.T) {
	...
	if act, _ := Reconcile(f, c, false); act != ActionStarted || !f.strt || f.pulled {
